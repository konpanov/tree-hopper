...
func TestNavigationStart(t *testing.T) {
	win := createWindow("assets/hello_world.txt")
	win.content = "qwe\nasd"
	if win.cursor.pos != 0 {
		t.Fail()
...
func TestNavigationDown(t *testing.T) {
	...
	cursorDown(win)
	expected := 4
	if win.cursor.pos != expected {
		t.Errorf("%d != %d", win.cursor.pos, expected)
	}
}

func TestNavigationDownOnDoubleNewLine(t *testing.T) {
	win := createWindow("assets/hello_world.txt")
	win.content = "qwe\n\nasd"
	cursorDown(win)
	expected := 4
...
func TestNavigationDownAndUpOnNewLine(t *testing.T) {
	win := createWindow("assets/hello_world.txt")
	win.content = "qwe\n\nasd"
	cursorDown(win)
	cursorUp(win)
